...
Ext.define('TouchTreeGrid.controller.CommonController', {
    ...
    onMenuButtonRelease: function(button, e, eOpts) {
        // Use RELEASE event (vs. tap event ... see button handler for #menubtn) for handling button press and prevent default
        // browser behaviour which activates underlying elements off TAP event (e.g. if search field or some other component placed underneath)


        var nav = this.getMenunav();
        var closed = nav.isHidden();

        if (!Ext.os.is.Phone) {
            if (closed) {nav.show();}
            else {nav.hide();}

            var mycont = this.getMenucont();
            if (closed) {mycont.setWidth('80%');}
            else {mycont.setWidth('100%');}
        }
        else {
            if (closed) {nav.show();}
            var cont = Ext.get('menucont');
    ...
    onMenuOptListSelect: function(dataview, record, eOpts) {
        ...
        var numRecords, gridcont, grid, gridListItemId, text, swap, newcont, idx, menulist,
            cont, menucont, isLeaf, result, swapcont;
        ...
        menucont = commonController.getMenucont();
        swapcont = menucont.down('#menucontswap');
        menulist = commonController.getMenuopt().down('#menuoptlist');
    ...
    onOrientationChange: function(viewport, orientation, width, height) {
        ...
        var gridcont = this.getMenucont().down('#menucontswap').getActiveItem().down('touchtreegrid');
        if (!gridcont) {return;}
    ...
    onHelpImageTap: function(image, e, eOpts) {
        var isPhone = !Ext.isEmpty(Ext.os.is.Phone);  // needed since this function returns either undefined or true
        var menucont = commonController.getMenucont();
    ...
    launch: function() {
        ...
        /* Hack to look to see if setInfinite() function exists on one of the tree grids as means to determine framework version .. not sure if more direct method exists or not */
        var accgrid  = tasksController.getAccordionCont().down('#firstexamplelist');
        if (typeof accgrid.setInfinite != 'function') {  /* if not found then we are running 2.1 framework */
            touchVersion = '2.1';
        }

        // Pre-select menu selection 
        // lastSelectionText is global variable initialized in init() method as ''

        var list = this.getMenuopt().down('#menuoptlist');
        var idx = list.getStore().find('Text', 'Accordion');  // Initialize with Accordion example
        list.select(idx); 

        // Fully expand menu options for non-phones 
        if (!Ext.os.is.Phone) {
            this.onMenuOptExpandImageTap();
        } else {
            // Initialize menu in expanded state for phones   
            this.onMenuButtonRelease();
        }
    }
