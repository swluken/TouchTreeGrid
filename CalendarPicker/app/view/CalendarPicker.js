...
Ext.define('CalendarPicker.view.CalendarPicker', {
    ...
    initialize: function() {
        ...
        if (me.getIncludeCustomDatesLegend()) {
            ...
            legend =   {
                ...
                height: 'initial',  // SWL UPDATED 1/17/15 TO SUPPORT TOUCH 2.4 FOR PHONES
                html: '',
        ...
        var footerBtnTextOverrides = me.getFooterBtnTextOverrides();
        // Update default button label text as specified in pickerfooterbtns{} ... added for international text
        if (!Ext.isEmpty(customFooterItems.items) && !this.isObjectEmpty(footerBtnTextOverrides)) {
            for (i=0; i<customFooterItems.items.length; i++) {
                if (customFooterItems.items[i].textType==='DONE' && !Ext.isEmpty(footerBtnTextOverrides.DONE)) {
                customFooterItems.items[i].text = footerBtnTextOverrides.DONE;}
                if (customFooterItems.items[i].textType==='Clear' && !Ext.isEmpty(footerBtnTextOverrides.Clear)) {
                customFooterItems.items[i].text = footerBtnTextOverrides.Clear;}
                if (customFooterItems.items[i].textType==='Cancel' && !Ext.isEmpty(footerBtnTextOverrides.Cancel)) {
                customFooterItems.items[i].text = footerBtnTextOverrides.Cancel;}
            }
        }

        gridcont.setCustomFooterItems(customFooterItems);
        ...
        if (this.getIncludeFooter() && this.getUseIconsForExpCollapse()) {
            ...
            exp = gridcont.down('#touchtreegridexpand');
            if (!Ext.isEmpty(exp)) {
                exp.setText('');
                exp.setIconCls('arrow_down');
                exp.setPadding(0);
            }
            col = gridcont.down('#touchtreegridcollapse');
            if (!Ext.isEmpty(col)) {
                col.setText('');
        ...
        // Override Expand/Collapse button text if not using arrows and if defined in footerBtnTextOverrides config
        if (this.getIncludeFooter() && !this.getUseIconsForExpCollapse() && !Ext.isEmpty(footerBtnTextOverrides) &&
        (!Ext.isEmpty(footerBtnTextOverrides.Expand) || !Ext.isEmpty(footerBtnTextOverrides.Collapse)) ) {

            exp = gridcont.down('#touchtreegridexpand');
            if (!Ext.isEmpty(exp) && !Ext.isEmpty(footerBtnTextOverrides.Expand)) {
                exp.setText(footerBtnTextOverrides.Expand);
            }
            col = gridcont.down('#touchtreegridcollapse');
            if (!Ext.isEmpty(col) && !Ext.isEmpty(footerBtnTextOverrides.Collapse)) {
                col.setText(footerBtnTextOverrides.Collapse);
            }
        }
        ...
        if (me.getAllowMonthAdditions() && monthsToInsert>0 ) {
            ...
            gridlist.setPlugins({
                ...
                pullText:       me.getMonthsToInsertPullText(),
                releaseText:    me.getMonthsToInsertRefreshText(),
                lastUpdatedText:       me.getMonthsToInsertLastUpdatedText(),
                lastUpdatedDateFormat: me.getMonthsToInsertLastUpdatedDateFormat(),
                loadedText:            me.getMonthsToInsertLoadedText(),
                loadingText:           me.getMonthsToInsertLoadingText(),
                listeners: {
    ...
    matrixPrep: function(parentConfig, addMode) {
        ...
        if  (matrixContainer.config.layout !== 'fit') {
    ...
    createStore: function(me, gridcont, gridlist, expandCollapse) {
        ...
        var weekendIndexArr = me.getWeekendIndexArr();
        var disablePastDates = me.getDisablePastDates();
        var startDay = me.getStartDay();
        ...
        do {
            ...
            firstDayIdx = (Ext.Date.getFirstDayOfMonth(dt)+7-startDay)%7;  // Adjust for startDay <> 0
            lastDayIdx = (Ext.Date.getLastDayOfMonth(dt)+7-startDay)%7;    // Adjust for startDay <> 0
            ...
            dateArr.push(
            {
                ...
                ID: id,
                PARENT_ID: parId,
                dow0: Ext.Date.getShortDayName((0+startDay)%7), // 'Sun',
                dow1: Ext.Date.getShortDayName((1+startDay)%7), // 'Mon',
                dow2: Ext.Date.getShortDayName((2+startDay)%7), // 'Tue',
                dow3: Ext.Date.getShortDayName((3+startDay)%7), // 'Wed',
                dow4: Ext.Date.getShortDayName((4+startDay)%7), //'Thu',
                dow5: Ext.Date.getShortDayName((5+startDay)%7), // 'Fri',
                dow6: Ext.Date.getShortDayName((6+startDay)%7) // 'Sat'
            });
            id++;  // increment next id
            weekNum = 1;
            i = 1;
            ...
            do {  // Loop for all days in current month
                ...
                for (j=0; j<=6; j++) {
                    ...
                    (dtStr>=Ext.Date.format(lastDtIdx, 'Y-m-d') && j>lastDayIdx)) {
                        ...
                        (disableWeekends && (weekendIndexArr.indexOf(parseInt(Ext.Date.format(dt, 'w'))) !== -1)) ||
    ...
    onLeafItemTap: function(me, list, index, target, record, e) {
        ...
        var weekendIndexArr = dtPkr.getWeekendIndexArr();
        ...
        if (tgt.hasCls('calendarpicker-disabled')) {return;}  // do nothing for disabled days
            ...
            if (selArr.length > 1) {
                ...
                if (!Ext.isEmpty(minDt) && !Ext.isEmpty(maxDt) && (minDt !== maxDt)) {
                    ...
                    do {
                        ...
                        (!disableWeekends || (weekendIndexArr.indexOf(parseInt(Ext.Date.format(dact, 'w'))) === -1)) &&
